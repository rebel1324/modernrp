...
concommand.Add("urltex_clearcache", function()
	urltex.Cache = {}
	urltex.Queue = {}
end)
...
function urltex.GetMaterialFromURL(url, callback, skip_cache, shader, size, size_hack)
	...
	if type(callback) == "function" and not skip_cache and urltex.Cache[url] then
		...
		local mat = CreateMaterial("nutscript_" .. util.CRC(url .. SysTime()), shader)
	...
	local function start()
		...
		hook.Add("Think", id, function()
			...
			if go and time < RealTime() then
				local vertex_mat = CreateMaterial("nutscript_" .. util.CRC(url .. SysTime()), "UnlitGeneric")
